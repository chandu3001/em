...
class DashboardController extends Controller
{
    ...
    function overview($school_id, Request $request)
    {
        ...
        $total_students_attended_the_exam = StudentsAnswersSub::select('students_answers_subs.student_id')->join('sql_dsms_mentric.students', 'students_answers_subs.student_id', 'students.id')
            ->join('examinations', 'students_answers_subs.exam_id', 'examinations.id')->where('result', '!=', '');

        $total_students_passed = StudentsAnswersSub::select('students_answers_subs.student_id')->join('sql_dsms_mentric.students', 'students_answers_subs.student_id', 'students.id')
            ->join('examinations', 'students_answers_subs.exam_id', 'examinations.id')->where('result', 'pass');
        $total_students_failed = StudentsAnswersSub::select('students_answers_subs.student_id')->join('sql_dsms_mentric.students', 'students_answers_subs.student_id', 'students.id')
            ->join('examinations', 'students_answers_subs.exam_id', 'examinations.id')->where(function ($condition) {
                $condition->where('result', 'fail');
            });
        ...
        if ($school_id) {
            $total_students_attended_the_exam = $total_students_attended_the_exam->where('result', '!=', '')->where('examinations.school_id', $school_id);
            $total_students_passed = $total_students_passed->where('result', '!=', '')->where('examinations.school_id', $school_id);
            $total_students_failed = $total_students_failed->where('result', '!=', '')->where('examinations.school_id', $school_id);
        }
        ...
        if ($school_id) {
            $latestExams = Examination::select('examinations.id', 'examinations.status', 'examinations.exam_name')->join('students_answers_subs', 'examinations.id', 'students_answers_subs.exam_id')->where('school_id', $school_id)->whereDate('students_answers_subs.created_at', '>=', $fromDate)->whereDate('students_answers_subs.created_at', '<=', $todate)->groupby('id', 'status', 'exam_name')->take(3)->get();
        } else {
            $latestExams = DB::table('schools')->selectRaw('schools.id, schools.name, cities.name as city_name')
                ...
                ->join('sql_em_mentric_test22.examinations', 'schools.id', 'examinations.school_id')
                ->join('sql_em_mentric_test22.students_answers_subs', 'examinations.id', 'students_answers_subs.exam_id')
                ->whereDate('students_answers_subs.created_at', '>=', $fromDate)->whereDate('students_answers_subs.created_at', '<=', $todate)
        ...
        foreach ($latestExams as $exam) {
            $data = StudentsAnswersSub::where('exam_id', $exam->id)->join('sql_dsms_mentric.students', 'students_answers_subs.student_id', 'students.id');
            $attended = clone $data;
            $passed = clone $data;
            $failed = clone $data;
