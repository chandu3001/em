...
class ExamCriteriaController extends Controller
{
    ...
    {
        $error = false;
        $data = DB::table('sql_em_mentric_test22.exam_criterias')
    ...
    public function update(Request $request, ExamCriteria $examCriteria)
    {
        ...
        if ($request->has('family_options') && $request->family_options) {
            ...
            $examCriteria->update([
                ...
                "family_questions" => (isset($request->family_options) && $request->family_options) ? json_encode($request->family_options) : $examCriteria->family_questions,
                "difficulty_questions" => (isset($request->difficulty_options) && $request->difficulty_options) ? json_encode($request->difficulty_options) : $examCriteria->difficulty_questions,
            ]);
        }
