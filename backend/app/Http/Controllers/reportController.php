...
class reportController extends Controller
{
    function index($school_id, Request $request)
    {
        ...
        $data = DB::table('sql_em_mentric_test22.students_answers_subs')
            ...
            ->join("sql_em_mentric_test22.languages", "students_answers_subs.language_id", "languages.id")
    ...
    {
        if ($request->has('school_ids') && $request->school_ids && count(json_decode($request->school_ids)) == 1 && json_decode($request->school_ids)[0][0] == 0) {
            $data = DB::table('license_types')->select('id', 'name')->whereNull('parent_id')->where('registration_status', 1)->get();
            return response()->json(["status" => true, "data" => $data], 200);

        } else if ($request->has('school_ids') && $request->school_ids && count(json_decode($request->school_ids)) > 0) {
