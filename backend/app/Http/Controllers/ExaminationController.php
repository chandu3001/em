...
class ExaminationController extends Controller
{
    ...
    function getExamResult($exam_id, Request $request)
    {
        if ($request->has('student_id')) {
            $data = DB::table('sql_em_mentric_test22.students_answers_subs')
                ...
                ->join("sql_em_mentric_test22.examinations", "examinations.id", "students_answers_subs.exam_id")
    ...
    {
        $data = Examination::select('licence_id', 'sub_license_id', 'exam_name', 'school_id')->find($id);
        if (!$data) {
        ...
        $defaultLanguage = Language::select('language_name')->where('school_id', $data->school_id)->where('default_language', 1)->get()->first();
        if($defaultLanguage)
        {
            $defaultLanguage = $defaultLanguage->language_name;
        }
        else
        {
            $defaultLanguage = '';
        ...
        return response()->json(["status" => true, "filter_data" => compact('families', 'difficulty_levels'), "total_family" => count($family_ids), "total_difficulty_levels" => count($difficulty_ids), "data" => $data2, "license_name" => $license_name, "sub_license_name" => $sub_license_name, "exam_name" => $exam_name, 'default_language' => $defaultLanguage]);
    ...
    public function show($school_id, Examination $examination, Request $request)
    {
        if ($school_id) {
            ...
            $data = DB::table('sql_em_mentric_test22.examinations')
        ...
        foreach ($data as $item) {
            ...
            $item->total_attended_students = StudentsAnswersSub::where('exam_id', $item->id)
            ->join('sql_dsms_mentric.students', 'students_answers_subs.student_id', 'students.id')->where('result', '!=', '')->count();
            $temp = StudentsAnswersSub::select('student_id')->where('exam_id', $item->id)
            ->join('sql_dsms_mentric.students', 'students_answers_subs.student_id', 'students.id');
