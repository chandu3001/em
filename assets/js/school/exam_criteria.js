...
if(langCode == 2){
	...
	function loadExamCriteriaModal(event, type = 'edit') {
		...
		if(langCode == 2){
			editTitle = "تحديث";;
			$('.save-nxt').html("حفظ والتالي");
			savenxtTitle = "حفظ والتالي";
			avaQues = "الأسئلة الاقصائية المتاحة";
			noOfQues = "مجموع الاسئلة";
		}else{
			editTitle = "Update";
			$('.save-nxt').html("Save & Next");
			savenxtTitle ="Save & Next";
			avaQues = "Available eliminatory questions";
			noOfQues =  "No of Questions";
		}
		$.ajax({
			...
			if (response.status == true) {
				...
				examCriteriaForm
					...
					.html(editTitle);
				type == 'edit' ? examCriteriaForm.find("#available_elimentary_questions").html(avaQues + ' ' + response.data.total_eliminatory_questions) : ''
				examCriteriaForm
					.find('.save__button button')
					.html(type == "view" ? "Next" : savenxtTitle);
				...
				if ((response.data.sub).length > 0) {
					...
					(response.data.sub).forEach(async (option, oindex) => {
										...
										<div class="form-group">
											<div class="form-control-wrap">
												<div class="d-flex align-items-center">
													...
													<div class="btn-light btn rounded-0 w-25 p-1 border-2 border-start-0 rounded-end"><span class="nos">Nos</span></div>
						...
						${type == 'edit' ? `<p class="diff_questions">${noOfQues} ${diff.total_available_questions}</p>` :''}
					</div>
						`)
						})
