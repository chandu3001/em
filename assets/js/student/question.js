		...
		jQuery.each(items, (index, item) => {
				...
				jQuery.each(item.options, (index, option) => {
	                  ...
	                  option_html +='<div class="form-group">'+
	                    '<div class="form-check">'+
							...
							'<label for="question-'+j+'-option'+i+'" class="form-check-label">'+option.option_name+'</label>'+
	                    '</div>'+
	                '</div>';
	                i++;         
	            });
				...
				$("#assessment").append(`<div class="col-md-12 eval_tab" id="eval_quest`+j+`" style="`+dispClass+`">
					...
					<form method="post" class="quizForm_`+j+`" id="quiz" name="quizForm">
							<fieldset class="mb-4 ques-list question-step question-step-`+j+ ` ques-list " id="question-step-`+j+`">
	                            
								<div id="question-`+j+`">
									<div class="qhscrol">
										<h5 class='d-flex justify-content-between'><span class="voice-over-`+j+`">${item.question}</span>

	                   					<div class=" border-gray200 p-15 text-gray">`+j+`/`+totQue+`</div>

										</h5>
										<p class="text-gray font-14 text-question mt-0">
	                		...
	                		<input type="hidden" name="question-pool-id`+j+`" value="`+item.id+`"/>
		      				...
		      				<button id="play"  class="VOICE btn btn-sm btn-warning mr-20" data-voice="`+j+`" data-coption="${item.options.length}" type="submit">Play Audio</button>
			...
			$('.VOICE').click(function(e){
			    e.preventDefault();
				var voiceVal = $(this).attr("data-voice");
				let optionCount = $(this).attr("data-coption");
				...
				for(let i=1; i<=optionCount; i++)
				{
					...
					const speech = optionText+$(`[for="question-${voiceVal}-option${i}"]`).text();
					speak(speech);
			...
			for(k=1;k<=totQue;k++){
				...
				//   }
				$('#que-analysis .pagination.options').append(`<div class="page-item sq not-done"  id="question-nav-${k}" >
					<a class="page-link" href="javascript:void(0)" onclick="questionIs(${k})">${k}</a>
				</div>`);
			}
			//end
			...
			$(".previous1, .next1, .pagination-number ul .page-link, [data-btn-val='finish']").click(function(){
...
console.log('start time'+valuestart+'stop time'+valuestop);
            ...
            console.log('seconds'+secondConverted);
			...
			if(isHrExist == 1){
				...
				examDuration =  minutesIs + ':' + secondConverted;
