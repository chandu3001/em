		...
		jQuery.each(items, (index, item) => {
			...
			$('.VOICE').click(function(e){
				...
				for(let i=1; i<=optionCount; i++)
				{
					...
					const speech = optionText+$(`[for="question-${voiceVal}-option${i}"] h6`).text();
					speak(speech);
			...
			// $('#que-analysis .pagination').pagination({
			// 	items: 28,
        	// 	itemsOnPage: 1,
			// });
			...
			for(k=1;k<=totQue;k++){
	        //    if(totQue <= 6) {
					$('#que-analysis .pagination.options').append(`<li class="page-item sq not-done" id="question-nav-${k}">
							        ...
							        $.ajax({
											...
											console.log('finished success');
												...
												if(response.data.result == 'pass'){
												    window.location.href = base_url + "student/live_exam/"+btoa('passed');
												}else{
													window.location.href = base_url + "student/live_exam/"+btoa('failed');
												}
