...
class StudentsController extends Controller
{
    ...
    function getValidLanguages($license_id, $sub_license_id)
    {
        ...
        $difficulty_questions = json_decode($criteria->difficulty_questions);
        $family_questions = json_decode($criteria->family_questions);

        $diff_ids = [];
        $family_ids = [];

        foreach ($difficulty_questions as $arr) {
            $id = DifficultyLevelMarks::find($arr->difficulty_id);
            if ($id) {
                array_push($diff_ids, $id->level_id);
            }
        }

        foreach ($family_questions as $ar) {
        ...
        foreach ($languages as $lang) {
            $temp = MainQuestionPool::join('languages_question_pools', 'main_question_pools.id', 'languages_question_pools.main_question_pool_id')
                ...
                ->where('language_id', $lang)->count();
            if ($criteria->no_of_elimentary_questions > 0) {
                $temp2 = MainQuestionPool::join('languages_question_pools', 'main_question_pools.id', 'languages_question_pools.main_question_pool_id')
                    ->whereIn('family_id', $family_ids)
            ...
            if ($total >= $criteria->total_questions) {
