    ...
    if($(this).val())
    {
      ...
      function getLicenseList2old()
      {
         $.ajax({
            ...
            if(!errors)
            {
               $("#school_license_filter").html('').append('<option value="0" selected>Select License</option>');
               data.result.forEach(item =>{
                  $("#school_license_filter").append(`<option value="">Choose license</option><option value="${item.id}">${item.name}</option>`)
               })
            }
            else
            {
...
function getLicenseList2() {
      $.ajax({
         ...
         if (!errors) {
             langCode = localStorage.getItem('language-type');
               languageText(langCode);
            $("#school_license_filter").html(`<option value=""></option>`);

            data.result.forEach(item => {
               $("#school_license_filter").append(`
      ...
      $('#exam-id').on('change', function() {
            langCode = localStorage.getItem('language-type');
            languageText(langCode);
             if ($('#exam-id :selected').length == 0){
               $('.select2-search__field').show();
             }else{
               $('.select2-search__field').hide();
             }

          //$('.select2-search__field').attr('placeholder','hELLOO');
      });

      $('#school_license_filter').on('change', function() {
           selval =  this.value ;
    ...
    $(".reset").on("click", function() {
         $("#student-id,#gender,#exam-id,#school_license_filter,#sub-license-filter").empty();
         $('#student-id').select2('val','null');
         $('#gender').select2('val','null');
         $('#exam-id').select2('val','null');
         $('#school_license_filter').select2('val','null');
     ...
     $('#clear-filter').click(function(){
      ...
      $('#school_license_filter').select2('val','0');
