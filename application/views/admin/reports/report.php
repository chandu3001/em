                                                                        ...
                                                                        <div class="form-group">
                                                                           <label
                                                                              ...
                                                                              <select name="sub_id"
                                                                                 ...
                                                                                 class="form-select form-select-sm js-select2"
                                                                                 data-placeholder="Select Sub License">
                                                                              </select>
                                                                        </div>
                                                                     </div>
                                                                       <div class="col-6">
    ...
    $.ajax({
        ...
        if (status) {
          langCode = localStorage.getItem('language-type');
               languageText(langCode);
            $("#admin_license_filter").html(`<option value=""></option>`);

            data.forEach(item => {
               $("#admin_license_filter").append(`
...
$("#admin_license_filter").on("change", function () {
    selval = this.value;
...
const getSubLicenseList = (target = false, license_id) => {
    ...
    $(target).html("").append('<option value="">xx</option>');
...
const getStudentListByGender = (target = false, school_id, gender) => {
   ...
   langCode = localStorage.getItem('language-type');
   //languageText(langCode);
      if(langCode == 2){ console.log('gender'+langCode)
               $('.select2-search__field').attr('placeholder','حدد');
            ...
            if (status) {
               ...
               `);
                    ...
                    data.data.forEach((item) => {
                        ...
                        examDate = moment(item.date_time).format('DD-MM-YYYY hh:mm:ss A');


                        if (item.result == "pass") {
                            resultData = `<button  type="button" class="btn w-90 d-flex justify-content-center btn-success tpass" style="background:#1abe92;padding: 4px 14px;border:none;">Pass</button>`;
                            addTextClr = "color:#e85347;";
                        } else {
                            resultData = `<button  type="button" class="btn w-90 d-flex justify-content-center btn-danger failed-title" style="border:none;padding:4px 17px;">Fail</button>`;
                  ...
                  <div class="nk-tb-item">
                     ...
                     <div class="nk-tb-col tb-col-md ltr">${examDate}</div>
...
$("#clear-filter").click(function () {
    ...
    $("#admin_license_filter").select2("val", "0");
